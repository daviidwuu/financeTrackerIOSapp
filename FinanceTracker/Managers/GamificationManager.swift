...
class GamificationManager: ObservableObject {
    ...
    var allMissions: [Mission] {
        ...
        let remoteMissions = FirebaseManager.shared.getRemoteMissions().compactMap { dict -> Mission? in
            guard let id = dict["id"] as? String,
                  let title = dict["title"] as? String,
                  let description = dict["description"] as? String,
                  let icon = dict["icon"] as? String,
                  let phase = dict["phase"] as? Int,
            ...
            return Mission(
                ...
                phase: phase,
                points: points,
                actionLink: dict["actionLink"] as? String,
                steps: dict["steps"] as? [String] ?? [],
                tutorialSteps: [] // Remote missions use simple steps for now
            )
        }
        
        return staticMissions + remoteMissions
    }
    ...
    private func updateCurrentPhase() {
        ...
        let phase1Complete = !phase1Missions.isEmpty && phase1Missions.allSatisfy { completedMissionIds.contains($0.id) }
        let phase2Complete = !phase2Missions.isEmpty && phase2Missions.allSatisfy { completedMissionIds.contains($0.id) }
        let phase3Complete = !phase3Missions.isEmpty && phase3Missions.allSatisfy { completedMissionIds.contains($0.id) }
